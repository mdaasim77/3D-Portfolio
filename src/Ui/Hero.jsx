...
export default function Hero({ onExplore, scrollRef, onProductsVisible }) {
  const titleRef = useRef();
  const subRef = useRef();
  const btnRef = useRef();
  const heroRef = useRef();
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) {
          gsap.to(heroRef.current, { opacity: 0, y: -30, duration: 0.5 });
          if (btnRef.current) btnRef.current.style.pointerEvents = "none";
          onProductsVisible(true);   // ← tell App products visible
        } else {
          gsap.to(heroRef.current, { opacity: 1, y: 0, duration: 0.5 });
          if (btnRef.current) btnRef.current.style.pointerEvents = "auto";
          onProductsVisible(false);  // ← tell App products hidden
        }
      },
      { threshold: 0.1 }
    );
