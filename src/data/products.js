const PRODUCTS = [
  {
    id: 1,
    name: "Apple MacBook Pro",
    price: "$1,999",
    description: "Next-generation performance. M3 chip. All day battery life.",
    model: "/models/macbook.glb",
    scale: 7,
    hotspots: [
      {
        id: 1,
        label: "Key Board",
        position: { top: "60%", left: "10%" },
        cameraX: 0,
        cameraY: 1,
        cameraZ: 1, // ← zoom to chip area
        info: "Magic Keyboard with Touch ID features a sleek, wireless design with a rechargeable battery that lasts about a month or more between charges.",
      },
      {
        id: 2,
        label: "Retina Display",
        position: { top: "60%", left: "40%" },
        cameraX: 0,
        cameraY: 0.4,
        cameraZ: 3,
        info: "Liquid Retina XDR display with 1000 nits sustained brightness.",
      },
    ],
  },
  {
    id: 2,
    name: "AirPods Max",
    price: "$549",
    description:
      "High-fidelity audio. Active noise cancellation. All day comfort.",
    model: "/models/airpods_max.glb",
    scale: 1,
    hotspots: [
      {
        id: 1,
        label: "ANC",
        position: { top: "25%", left: "25%" },
        cameraX: 1.5,
        cameraY: 1,
        cameraZ: 1,
        info: "Active Noise Cancellation blocks outside noise using advanced algorithms.",
      },
      {
        id: 2,
        label: "Audio Engine",
        position: { top: "70%", left: "25%" },
        cameraX: 1.5,
        cameraY: -0.5,
        cameraZ: 1,
        info: "Apple-designed dynamic driver delivers high-fidelity audio.",
      },
    ],
  },
  {
    id: 3,
    name: "Mac Studio",
    price: "$2,999",
    description: "Incredible performance in a compact design. M2 Max chip.",
    model: "/models/macstudio.glb",
    scale: 0.7,
    hotspots: [
      {
        id: 1,
        label: "M2 Max",
        position: { top: "60%", left: "10%" },
        cameraX: -3,
        cameraY: 2,
        cameraZ: 2.5,
        info: "M2 Max chip with 12-core CPU and 38-core GPU for pro workflows.",
      },
      {
        id: 2,
        label: "Ports",
        position: { top: "60%", left: "40%" },
        cameraX: 7,
        cameraY: 1,
        cameraZ: 0,
        info: "Front ports include 2x USB-C and SDXC card slot for fast transfers.",
      },
    ],
  },
  {
    id: 4,
    name: "Apple Watch",
    price: "$699",
    description: "Most rugged Apple Watch. Precision dual-frequency GPS.",
    model: "/models/watch.glb",
    scale: 0.5,
    hotspots: [
      {
        id: 1,
        label: "Display",
        position: { top: "35%", left: "10%" },
        cameraX: 0,
        cameraY: 0.3,
        cameraZ: 1.8,
        info: "Always-On Retina display with up to 2000 nits brightness outdoors.",
      },
      {
        id: 2,
        label: "Dial",
        position: { top: "35%", left: "40%" },
        cameraX: 2.5,
        cameraY: 0,
        cameraZ: 1.5,
        info: "Allows you to scroll through menus, zoom in and out on photos, adjust volume, and return to the main watch face.",
      },
    ],
  },
  {
    id: 5,
    name: "Apple iPad",
    price: "$799",
    description: "M2 chip. Liquid Retina XDR display. All-day battery.",
    model: "/models/ipad.glb",
    scale: 2,
    hotspots: [
      {
        id: 1,
        label: "Camera",
        position: { top: "50%", left: "10%" },
        cameraX: 0,
        cameraY: 0,
        cameraZ: 2.5,
        info: "The iPad camera varies significantly across models, with newer versions offering advanced features.",
      },
      {
        id: 2,
        label: "Speaker",
        position: { top: "50%", left: "40%" },
        cameraX: 2,
        cameraY: -10,
        cameraZ: 6,
        info: "The iPad's built-in speakers vary in quality depending on the model.",
      },
    ],
  },
  {
    id: 6,
    name: "Apple iPhone",
    price: "$1,299",
    description: "Titanium design. A17 Pro chip. Pro camera system.",
    model: "/models/iphone.glb",
    scale: 10,
    hotspots: [
      {
        id: 1,
        label: "Button",
        position: { top: "50%", left: "10%" },
        cameraX: 6,
        cameraY: -0.3,
        cameraZ: 2.5,
        info: "The iPhone 17 series features several new and enhanced physical buttons, each with customizable functions",
      },
      {
        id: 2,
        label: "Volume button",
        position: { top: "50%", left: "40%" },
        cameraX: -6,
        cameraY: 0.5,
        cameraZ: 2.5,
        info: "The volume buttons on the iPhone 17 are located on the left side of the device, just above the Action button.",
      },
    ],
  },
];

export default PRODUCTS;
